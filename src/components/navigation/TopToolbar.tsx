...
import { Search, Layers, Box, RotateCcw, ChevronDown, Sparkles, LayoutDashboard } from "lucide-react";
import { useMachineStore } from "../../store/machineStore";
import { machineInfo } from "../../data/machineData";
...
export function TopToolbar() {
  ...
  return (
    <header className="relative z-30 flex h-14 shrink-0 items-center gap-4 border-b border-industrial-border bg-industrial-panel px-4">
      ...
      <button className="hidden items-center gap-2 rounded-md border border-industrial-border bg-industrial-panel-alt px-2.5 py-1.5 text-sm text-industrial-text hover:border-industrial-accent/50 md:flex">
        <span className="font-medium">{machineInfo.tag}</span>
        <span className="text-industrial-muted">{machineInfo.name.replace(machineInfo.tag, "").trim()}</span>
      ...
      <div className="flex items-center gap-1.5">
        <ToolbarButton label="Layers" active={layersPanelOpen} onClick={toggleLayersPanel} icon={<Layers size={16} />} />
        <ToolbarButton label="Exploded View" active={explodedView} onClick={toggleExplodedView} icon={<Box size={16} />} />
        <ToolbarButton label="Dashboard" active={dashboardOpen} onClick={toggleDashboard} icon={<LayoutDashboard size={16} />} />
        <ToolbarButton label="AI Assistant" active={assistantOpen} onClick={toggleAssistant} icon={<Sparkles size={16} />} />
        <ToolbarButton label="Reset View" onClick={resetView} icon={<RotateCcw size={16} />} />
      </div>
    </header>
  );
}
